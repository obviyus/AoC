...
def part_1():
    with open("2021/inputs/day4", "r") as file:
        bingo_numbers = list(map(int, file.__next__().strip().split(",")))
        print(bingo_numbers)

        results, current_board, current_marked = (
            [],
            [],
            [],
        )  # (solved_in, sum_of_unmarked, final_number)
        for line in file:
            if line.strip():
                ...
                for val in bingo_numbers:
                    if check_if_present(current_board, val):
                        current_marked[check_if_present(current_board, val)[0]][
                            check_if_present(current_board, val)[1]
                        ] = 1
